...
export class NishthaDashboardComponent implements OnInit {
  ...
  getToken(): Observable<any> {
    //dev url
    // const URL =
    //   "https://dev.oci.diksha.gov.in/auth/realms/sunbird/protocol/openid-connect/token";
    // Prod url
    ...
    // client_id=ntp-support-tool&username=reportviewer_tn@yopmail.com&grant_type=password&password=&client_secret=7052d78c-7609-4ddd-958c-420b277307bd  // dev
    ...
    if (this.isUserLoggedIn) {
      this.getToken().subscribe((res) => {
        if (res.access_token) {
          ...
          this.siteUrl =
            "https://course-data.diksha.gov.in/login/?diksha_token=" +
            token +
            "&redirect=/bi/dashboard/course-dashboard/?standalone=1&show_filters=0";
  ...
  sendInteractDataToTelemetry(uniqueId) {
    const data = {
      context: {
        env: "Prod - Course Dashboard",
        cdata: [],
      },
      edata: {
        id: uniqueId,
        type: "Course Dashboard click interact",
        pageid: this.router.url.split("?")[0],
        subtype: "Prod - Course Dashboard",
      },
    };
